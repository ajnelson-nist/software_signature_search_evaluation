...
WHERE
  ...
  a.n_grams = "1" AND
  a.stop_list_n_gram_strategy = "raw_filter" AND
  a.sequences = "installclose" AND
  a.combinator = "summation" AND
  a.stop_list = "none" AND
  a.score_selector = "min"
ORDER BY
  2 * (evaluation_recall * m57_recall) / (evaluation_recall + m57_recall) DESC,
