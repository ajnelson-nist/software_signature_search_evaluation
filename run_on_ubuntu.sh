...
time make -j $NUM_JOBS GTIME=/usr/bin/time SEVENZIP=7zr magnum_opus.mk mult_results.mk
...
TMPDIR="$PWD/tmp" time make --keep-going -j $NUM_JOBS GTIME=/usr/bin/time demo-prereqs >make.demo-prereqs.log 2>&1
date
TMPDIR="$PWD/tmp" time make --keep-going -j $NUM_JOBS GTIME=/usr/bin/time >make.all.log 2>&1 || time make >make.all.fail.log 2>&1
date
